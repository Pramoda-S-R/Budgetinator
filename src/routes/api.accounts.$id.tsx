    ...
    handlers: {
      PATCH: async ({ request, params }) => {
        ...
        const updates: Partial<typeof accounts.$inferInsert> = {
          ...
          ...('accountType' in parsedBody.data ? { accountType: parsedBody.data.accountType } : {}),
          ...('includeInNetWorth' in parsedBody.data
            ? { includeInNetWorth: parsedBody.data.includeInNetWorth }
            : {}),
          ...('isActive' in parsedBody.data ? { isActive: parsedBody.data.isActive } : {}),
          ...('currentBalance' in parsedBody.data
            ? { currentBalance: toNumericString(parsedBody.data.currentBalance as number) }
            : {}),
        }
        ...
        if ('currentBalance' in parsedBody.data && typeof parsedBody.data.currentBalance === 'number') {
