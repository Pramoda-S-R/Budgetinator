    ...
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        const [group] = await db
          ...
          .from(categoryGroups)
          .where(and(eq(categoryGroups.id, parsed.data.groupId), eq(categoryGroups.userId, user.id)))
          .limit(1)
        ...
        const [lastCategory] = await db
          ...
          .from(categories)
          .where(and(eq(categories.userId, user.id), eq(categories.groupId, parsed.data.groupId)))
