    ...
    handlers: {
      GET: async ({ request }) => {
        ...
        const [{ netWorth }] = await db
          .select({ netWorth: sql`coalesce(sum(${accounts.currentBalance}), 0)` })
          .from(accounts)
          .where(
            and(
        ...
        const [{ currentCash }] = await db
          .select({ currentCash: sql`coalesce(sum(${accounts.currentBalance}), 0)` })
          .from(accounts)
          .where(
            and(
              eq(accounts.userId, user.id),
        ...
        const savingsRate = String(totalIncome) === '0'
          ? '0'
          : (
            (parseFloat(monthlySavings) / parseFloat(String(totalIncome)) * 100)
          ).toFixed(2);
        ...
        const [{ totalInvestments }] = await db
          .select({ totalInvestments: sql`coalesce(sum(${accounts.currentBalance}),0)` })
          .from(accounts)
          .where(
            and(
              eq(accounts.userId, user.id),
        ...
        const investmentRatio = String(netWorth) === '0'
          ? '0'
          : (
            (parseFloat(String(totalInvestments)) / parseFloat(String(netWorth)) * 100)
          ).toFixed(2);
