...
function CategoriesPage() {
  ...
  const [categoryTransactionType, setCategoryTransactionType] = useState('expense')
  ...
  const groups = useMemo(
    () => [...(categoryGroupsQuery.data?.categoryGroups ?? [])].sort((a, b) => a.sortOrder - b.sortOrder),
  ...
  const createGroupMutation = useMutation({
    mutationFn: (input: { name: string; type: string; icon: string; color: string }) =>
      createCategoryGroup(input, currentUser),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['category-groups', currentUser?.id] })
      setGroupName('')
    },
  })
  ...
  const createCategoryMutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['categories', currentUser?.id] })
      setCategoryName('')
    },
  })

  const updateGroupMutation = useMutation({
    mutationFn: (payload: { groupId: string; input: Parameters<typeof updateCategoryGroup>[1] }) =>
      updateCategoryGroup(payload.groupId, payload.input, currentUser),
  })

  const updateCategoryMutation = useMutation({
    mutationFn: (payload: { categoryId: string; input: Parameters<typeof updateCategory>[1] }) =>
  ...
  async function refreshCategoriesState() {
    await Promise.all([
      queryClient.invalidateQueries({ queryKey: ['category-groups', currentUser?.id] }),
      queryClient.invalidateQueries({ queryKey: ['categories', currentUser?.id] }),
    ])
  }
  ...
  async function reorderCategories(groupId: string, sourceId: string, targetId: string) {
    if (sourceId === targetId) {
      return
    }

    const groupCategories = [...(categoriesByGroupId[groupId] ?? [])].sort((a, b) => a.sortOrder - b.sortOrder)
    const sourceIndex = groupCategories.findIndex((category) => category.id === sourceId)
    const targetIndex = groupCategories.findIndex((category) => category.id === targetId)
  ...
  async function onGroupDrop(event: DragEvent<HTMLDivElement>, groupId: string) {
    event.preventDefault()
  ...
  async function onCategoryDrop(event: DragEvent<HTMLDivElement>, groupId: string, categoryId: string) {
    event.preventDefault()
  ...
  return (
    <div className="space-y-6 p-6">
        ...
        <CardContent>
          <form onSubmit={onCreateGroup} className="grid gap-3 md:grid-cols-5">
            <div className="space-y-2">
              ...
              <Input id="group-name" value={groupName} onChange={(event) => setGroupName(event.target.value)} required />
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <IconSelect value={groupIcon} onChange={setGroupIcon} options={GROUP_ICON_OPTIONS} />
            </div>
            <div className="space-y-2">
      ...
      <Card>
        ...
        <CardContent>
          <form onSubmit={onCreateCategory} className="grid gap-3 md:grid-cols-6">
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                onChange={(event) => setCategoryTransactionType(event.target.value)}
                required
              >
                {TRANSACTION_TYPE_OPTIONS.map((option) => (
                  <option key={option.value} value={option.value}>
            ...
            <div className="space-y-2">
              ...
              <IconSelect value={categoryIcon} onChange={setCategoryIcon} options={CATEGORY_ICON_OPTIONS} />
            </div>
            <div className="space-y-2">
            ...
            <div className="md:col-span-6">
              <Button type="submit" disabled={createCategoryMutation.isPending || groups.length === 0}>
                Add Category
              </Button>
            </div>
          </form>
          ...
          {groups.map((group) => (
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  <p className="flex items-center gap-2 font-medium"><IconGlyph iconName={group.icon} />{group.name}</p>
                  <p className="text-xs text-muted-foreground">
                    {group.type} • {group.color}
                  </p>
                </div>
                <div className="flex gap-2">
                  <Button variant="outline" onClick={() => onToggleGroupArchive(group)}>
                    {group.isArchived ? 'Unarchive' : 'Archive'}
                  </Button>
                  <Button variant="destructive" onClick={() => onDeleteGroup(group.id)}>
                    Delete
                  </Button>
                </div>
              </div>
              ...
              <div className="space-y-2 pl-3">
                {(categoriesByGroupId[group.id] ?? []).length === 0 ? (
                  <p className="text-xs text-muted-foreground">No categories in this group.</p>
                ) : null}
                ...
                {(categoriesByGroupId[group.id] ?? [])
                  ...
                  .map((category) => (
                    <div
                      ...
                      onDragOver={(event) => event.preventDefault()}
                      onDrop={(event) => onCategoryDrop(event, group.id, category.id)}
                    >
                      <div>
                        <p className="flex items-center gap-2 text-sm font-medium"><IconGlyph iconName={category.icon} />{category.name}</p>
                        <p className="text-xs text-muted-foreground">
                          {category.transactionType} • {category.color}
                        </p>
                      </div>
                      <div className="flex gap-2">
                        <Button variant="outline" onClick={() => onToggleCategoryArchive(category)}>
                          {category.isArchived ? 'Unarchive' : 'Archive'}
                        </Button>
                        <Button variant="destructive" onClick={() => onDeleteCategory(category.id)}>
                          Delete
                        </Button>
                      </div>
                    </div>
