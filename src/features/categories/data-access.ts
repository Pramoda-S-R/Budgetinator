...
function createAuthHeaders(user?: User) {
  if (!user?.id) {
    return {}
  }
...
export async function fetchCategoryGroups(user?: User, includeArchived = false) {
  const search = includeArchived ? '?includeArchived=true' : ''

  return request<{ categoryGroups: CategoryGroup[] }>(`/api/category-groups${search}`, {
    headers: createAuthHeaders(user),
  })
}

export async function createCategoryGroup(input: CreateCategoryGroupInput, user?: User) {
...
export async function updateCategoryGroup(groupId: string, input: UpdateCategoryGroupInput, user?: User) {
  return request<{ categoryGroup: CategoryGroup }>(`/api/category-groups/${groupId}`, {
    method: 'PATCH',
    headers: {
      'content-type': 'application/json',
      ...createAuthHeaders(user),
...
export async function updateCategory(categoryId: string, input: UpdateCategoryInput, user?: User) {
