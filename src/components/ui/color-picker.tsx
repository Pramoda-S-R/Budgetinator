...
const VIBRANT_PRESETS = ['#FF4D6D', '#FF7A00', '#FFD60A', '#38B000', '#00B4D8', '#3A86FF', '#8338EC', '#FF006E']
...
export function ColorPicker({
  ...
  const normalized = HEX_COLOR_REGEX.test(value) ? value.toUpperCase() : '#64748B'
  ...
  const draftNormalized = HEX_COLOR_REGEX.test(draftColor) ? draftColor.toUpperCase() : '#64748B'
          ...
          <DialogContent className="rounded-none sm:max-w-sm">
            ...
            <div className="space-y-4">
              <div className="[&_.react-colorful]:w-full [&_.react-colorful__saturation]:rounded-none [&_.react-colorful__hue]:h-3 [&_.react-colorful__hue]:rounded-none [&_.react-colorful__pointer]:h-4 [&_.react-colorful__pointer]:w-4 [&_.react-colorful__pointer]:rounded-none [&_.react-colorful__pointer]:border-2 [&_.react-colorful__pointer]:border-background">
                <HexColorPicker color={draftNormalized} onChange={(nextColor) => setDraftColor(nextColor.toUpperCase())} />
              </div>
              <Input
                ...
                {VIBRANT_PRESETS.map((preset) => (
                  <button
                    ...
                    className={cn(
                      ...
                      draftNormalized === preset ? 'border-foreground' : 'border-input',
                    )}
